...
export const getNetworkMember = async (networkId: any, mem: string) => {
  return networkConfig[networkId][mem];
};
...
export const autoFundCheck = async (
  ...
  const balance = await hre.web3.utils.toBN(balanceHex._hex).toString();
  const contractBalanceHex = await linkTokenContract.balanceOf(contractAddr);
  const contractBalance = await hre.web3.utils
